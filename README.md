...
├── products/                   # 제품·서비스 하나당 폴더 하나 (KB 화면과 1:1)
│   ├── README.md               # 제품·서비스 목록 인덱스
│   ├── 작성가이드.md            # 담당자용 작성 가이드
│   ├── _양식/                   # 새 폴더 양식 (통째로 복사)
│   └── <제품·서비스명 한글>/
│       ├── README.md           # 제품 설명·기능 계층 (서비스는 수행 활동)
│       ├── slides.md           # 제안 슬라이드 인덱스
│       └── images/             # 화면 캡쳐 png
├── business/
│   ├── README.md               # 사업 영역 인덱스
...
| [products/데이터메타품질컨설팅/](./products/데이터메타품질컨설팅/) | 데이터 메타·품질 컨설팅 (서비스 사업) |
| [products/DW구축/](./products/DW구축/) | DW 구축 (서비스 사업) |

> 🖥️ 제안 현장에서 반복 요구받는 **제안 대응 문서 4종**(소개서·매뉴얼·사이징·제안대응)은
> 이 저장소가 아니라 영업 지원 스테프 화면(`제품·서비스 정보`)에서 입력합니다.
> [products/작성가이드.md 4장](./products/작성가이드.md#4-제안-대응-문서-4종--시스템-화면에서-입력합니다) 참조.
...
주요 사업: Tailored AX (WiseAIOps, WiseProphet, WiseIntelligence)
         Data Management (WiseDQ, WiseMeta)
...
| 서버 스펙·이중화 구성 변경 | 화면 `제품·서비스 정보 > 사이징` 탭 |
| 요구사항 수용 범위·공수 기준 변경 | 화면 `제품·서비스 정보 > 제안대응` 탭 |
| 소개서·매뉴얼 개정 | 화면 `제품·서비스 정보 > 소개서·매뉴얼` 탭 (버전·위치·갱신일) |
